...
public class DiretorioResource {
    ...
    public Response register(UserClass user){
        ...
        if(d==null) return Response.status(400).build();
        boolean b;
        synchronized (this){
        ...
        if (!b) return Response.status(201).build();
        else return Response.status(412).build();//verificar se e o suposto dar conflict
    }
    ...
    public Response infection(UserClass user){
        Distrito d = distritos.get(user.district);
        if(d==null) return Response.status(400).build();
        if(d.infectedUser(user.username)) return Response.ok().build();
    ...
    public Response contact(UserClass user){
        Distrito d = distritos.get(user.district);
        if(d==null) return Response.status(400).build();
        if(d.infContactUser(user.username)) return Response.ok().build();
        return Response.status(304).build();
    }
    ...
    public Response top5(TopClass pos){
        ...
        if(d==null) return Response.status(400).build();
        if (d.updateTop(pos.positionX, pos.positionY, pos.record)) return Response.ok().build();
        return Response.status(304).build();
    }
    ...
    public Response infectedRatio(){
        return Response.ok(new TreeSet<>(this.distritos.values()).stream().map(e->new InfRatio(e.name,e.infectedRatio())).limit(5).collect(Collectors.toList())).build();
    }
